...
#Collision checking for a path
	def inobstacle(self,x1,y1,x2,y2):
		...
		for i in range(1,round(obs_num)+1):
			xomin=self.x[4*(i-1)]
...
with open('filteredpcd.xyz') as f:
	coordinates = f.readlines()
#with open('environment_test copy.txt') as f:
#	coordinates = f.readlines()

vx = list(map(float, coordinates[0].split()))
vy = list(map(float, coordinates[1].split()))
vz = list(map(float, coordinates[2].split()))
